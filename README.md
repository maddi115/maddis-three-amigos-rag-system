...
-----Gradient Proximity Search: Identifies rarest query words as anchors, then it finds said anchors in the data then it radiates outward bidirectionally through where the important word was found then it will look for nearby query terms with exponentially decaying weights until it finds a strong query word in nearby which will further its strength, strength fades below threshold—stopping when it doesnt reach any other word that is in the query--its stops now include a bigger window to incldue the conversations in twitch chat which now may relate to the query and made threads 
